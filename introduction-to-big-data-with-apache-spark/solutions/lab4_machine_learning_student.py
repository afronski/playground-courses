...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    average = sum(IDandRatingsTuple[1]) / float(length)
    return (IDandRatingsTuple[0], (length, average))
...
movieIDsWithRatingsRDD = (ratingsRDD
                          ...
                          .combineByKey(empty, add, merge)
                          .map(lambda r: (r[0], tuple(r[1]))))
...
def computeError(predictedRDD, actualRDD):
    ...
    squaredErrorsRDD = (predictedReformattedRDD
                        ...
                        .map(lambda a: math.pow(a[1][0] - a[1][1], 2)))
    
    totalError = squaredErrorsRDD.fold(0, lambda a, b: a + b)
...
validationForPredictRDD = validationRDD.map(lambda r: (r[0], r[1]))
...
testForPredictingRDD = testRDD.map(lambda r: (r[0], r[1]))
...
trainingAvgRating = trainingRDD.map(lambda a: a[2]).fold(0, lambda a,b: a + b) / trainingRDD.count()
print 'The average rating for movies in the training set is %s' % trainingAvgRating

testForAvgRDD = testRDD.map(lambda r: (r[0], r[1], trainingAvgRating))
...
ratedIds = map(lambda r: r[1], myRatedMovies)
myUnratedMoviesRDD = (moviesRDD
                      .filter(lambda r: r[0] not in ratedIds)
                      .map(lambda r: (0, r[0])))

predictedRatingsRDD = myRatingsModel.predictAll(myUnratedMoviesRDD)
print predictedRatingsRDD.take(4)
...
movieCountsRDD = movieIDsWithAvgRatingsRDD.map(lambda r: (r[0], r[1][0]))
