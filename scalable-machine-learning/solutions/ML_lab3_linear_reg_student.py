...
onlyLabels = parsedDataInit.map(lambda r: r.label)
...
parsedData = parsedDataInit.map(lambda r: LabeledPoint(r.label - minYear, r.features))
...
def calcRMSE(labelsAndPreds):
    ...
    """
    return math.sqrt(labelsAndPreds.map(lambda (a, b): squaredError(a, b)).sum() / labelsAndPreds.count())
...
labelsAndPredsTrain = parsedTrainData.map(lambda r: (r.label, averageTrainYear))
rmseTrainBase = calcRMSE(labelsAndPredsTrain)

labelsAndPredsVal = parsedValData.map(lambda r: (r.label, averageTrainYear))
rmseValBase = calcRMSE(labelsAndPredsVal)

labelsAndPredsTest = parsedTestData.map(lambda r: (r.label, averageTrainYear))
...
labelsAndPreds = parsedValData.map(lambda r: getLabeledPrediction(weightsLR0, r))
...
labelsAndPreds = parsedValData.map(lambda r: (r.label, firstModel.predict(r.features)))
...
def twoWayInteractions(lp):
    ...
    """
    interactions = map(lambda (a,b): a * b, list(itertools.product(lp.features, lp.features)))
...
trainDataInteract = parsedTrainData.map(twoWayInteractions).cache()
valDataInteract = parsedValData.map(twoWayInteractions).cache()
testDataInteract = parsedTestData.map(twoWayInteractions).cache()
