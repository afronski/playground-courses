...
class BinaryTreeSuite(_system: ActorSystem)
  extends TestKit(_system) with FunSuite with ShouldMatchers with BeforeAndAfterAll with ImplicitSender {

  def this() =
    this(ActorSystem("PostponeSpec"))
  ...
  def receiveN(requester: TestProbe, ops: Seq[Operation], expectedReplies: Seq[OperationReply]): Unit =
    within(5 seconds) {
      val repliesUnsorted = for (i <- 1 to ops.size) yield try {
        ...
        case exception: Throwable if ops.size > 10 => {
          fail(s"failure to receive confirmation $i/${ops.size}", exception)
        }
      ...
      if (replies != expectedReplies) {
        ...
        fail("unexpected replies:" + pairs
                                      .map(x => s"at index ${x._2}: got ${x._1._1}, expected ${x._1._2}")
                                      .mkString("\n    ", "\n    ", ""))
      }
    }

  def verify(probe: TestProbe, operations: Seq[Operation], expected: Seq[OperationReply]): Unit = {
    ...
    receiveN(probe, operations, expected)
  }
  ...
  test("behave identically to built-in set (includes GC)") {
    ...
    def referenceReplies(operations: Seq[Operation]): Seq[OperationReply] = {
      ...
      def replyFor(operation: Operation): OperationReply = operation match {
      ...
      for (operation <- operations) yield replyFor(operation)
    }
    ...
    val operations = randomOperations(requester.ref, count)
    val expectedReplies = referenceReplies(operations)
    ...
    receiveN(requester, operations, expectedReplies)
  }
}